...
alias dev-build='cmake --build build --preset=dev-build -j $THREADS'
alias dev-all='dev-config && dev-build'

alias docs-config='cmake --preset=docs'
alias docs-build='cmake --build build/docs --preset=docs-build -j $THREADS'
...
alias tests-build='cmake --build build/tests --preset=tests-build -j $THREADS'
...
alias rel-build='cmake --build build/release --preset=release-build -j $THREADS'
...
alias vcpkg-init='git clone git@github.com:microsoft/vcpkg.git && cd ./vcpkg/ && ./bootstrap-vcpkg.sh && cd -'
