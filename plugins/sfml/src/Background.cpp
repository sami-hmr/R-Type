                                   ...
                                   JsonObject const& obj)
{
  auto const& textures_path =
      get_value<JsonArray>(this->_registry.get(), obj, "layers");
  ...
  for (const auto& layer : textures_path.value()) {
    ...
    const auto& path_str =
        get_value<std::string>(this->_registry.get(), path_obj, "path");
  ...
  auto const& render_type_str =
      get_value<std::string>(this->_registry.get(), obj, "render_type");
  ...
  const std::optional<JsonObject>& parallax_obj =
      get_value<JsonObject>(this->_registry.get(), obj, "parallax");
  if (parallax_obj.has_value()) {
    const auto& active =
        get_value<bool>(this->_registry.get(), parallax_obj.value(), "active");
    const auto& speed = get_value<JsonObject>(
        this->_registry.get(), parallax_obj.value(), "speed");
    const auto& framerate = get_value<double>(
        this->_registry.get(), parallax_obj.value(), "framerate");
