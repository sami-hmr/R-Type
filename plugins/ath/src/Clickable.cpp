...
void ATH::init_clickable(Ecs::Entity const& e, JsonObject const& obj)
{
  ...
  if (obj.contains("to_emit")) {
    try {
        ...
        try {
          ...
          for (auto const& [event_name, event_data_value] : emit_obj) {
            try {
              ...
              std::cerr << "Error parsing clickable emit: event data is not a "
                           "JsonObject"
                        << '\n';
            }
          }
      ...
      std::cerr
          << "Error parsing clickable component: 'to_emit' is not a JsonArray"
          << '\n';
    }
  }
  init_component<Clickable>(
      this->_registry.get(), this->_event_manager.get(), e, emits);
}
