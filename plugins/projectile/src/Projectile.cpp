...
void Projectile::init_owner(Registery::Entity entity, JsonObject const& obj)
{
  auto const& owner =
      get_value<Owner, int>(this->_registery.get(), obj, entity, "owner");
  ...
  this->_registery.get().emplace_component<Owner>(entity, owner.value());
}
