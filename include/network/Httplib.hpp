...
#  ifdef __linux__
#    include <resolv.h>
#    undef _res  // Undefine _res macro to avoid conflicts with user code
                 // (#2278)
#  endif
#  include <csignal>
...
template<typename T>
using unordered_set = std::
    unordered_set<T, detail::case_ignore::hash, detail::case_ignore::equal_to>;
...
struct Request
{
  ...
  std::string get_header_value(const std::string& key,
                               const char* def = "",
                               size_t id = 0) const;
  size_t get_header_value_u64(const std::string& key,
                              size_t def = 0,
                              size_t id = 0) const;
...
private:
  ...
  bool check_if_not_modified(const Request& req,
                             Response& res,
                             const std::string& etag,
                             time_t mtime) const;
  bool check_if_range(Request& req,
                      const std::string& etag,
                      time_t mtime) const;
...
public:
  ...
  std::string get_request_header_value(const std::string& key,
                                       const char* def = "",
                                       size_t id = 0) const;
  size_t get_request_header_value_u64(const std::string& key,
                                      size_t def = 0,
                                      size_t id = 0) const;
...
inline ssize_t read_body_content(Stream* stream,
                                 BodyReader& br,
                                 char* buf,
                                 size_t len)
{
...
private:
  ...
  bool write_request(Stream& strm,
                     ...
                     bool close_connection,
                     Error& error);
  void prepare_default_headers(Request& r,
                               bool for_stream,
                               const std::string& ct);
...
inline size_t Request::get_header_value_u64(const std::string& key,
                                            size_t def,
                                            size_t id) const
{
...
inline size_t Response::get_header_value_u64(const std::string& key,
                                             size_t def,
                                             size_t id) const
{
  return detail::get_header_value_u64(headers, key, def, id);
...
inline bool set_socket_opt_impl(
    socket_t sock, int level, int optname, const void* optval, socklen_t optlen)
{
...
inline bool set_socket_opt_time(
    socket_t sock, int level, int optname, time_t sec, time_t usec)
{
#ifdef _WIN32
...
inline std::wstring u8string_to_wstring(const char* s)
{
  ...
  wlen = ::MultiByteToWideChar(
      ...
      s,
      len,
      const_cast<LPWSTR>(reinterpret_cast<LPCWSTR>(ws.data())),
      wlen);
...
std::string make_host_and_port_string(const std::string& host,
                                      int port,
                                      bool is_ssl);
...
bool parse_trailers(stream_line_reader& line_reader,
                    Headers& dest,
...
// GET
template<typename ClientType>
inline Result Get(ClientType& cli,
                  const std::string& path,
...
// HEAD
template<typename ClientType>
inline Result Head(ClientType& cli,
                   const std::string& path,
                   size_t chunk_size = 8192)
{
...
// OPTIONS
template<typename ClientType>
inline Result Options(ClientType& cli,
                      const std::string& path,
                      size_t chunk_size = 8192)
{
...
inline bool parse_trailers(stream_line_reader& line_reader,
                           Headers& dest,
                           const Headers& src_headers)
{
...
inline bool split_find(const char* b,
                       const char* e,
                       char d,
                       size_t m,
...
{
  return split_find(
      b, e, d, (std::numeric_limits<size_t>::max)(), std::move(fn));
}
...
inline int getaddrinfo_with_timeout(const char* node,
                                    const char* service,
...
#  ifdef _WIN32
  ...
  if (ret == WSA_IO_PENDING) {
    ...
    if (!::GetOverlappedResult(
            (HANDLE)INVALID_SOCKET, &overlapped, &bytes_returned, FALSE))
    {
      ::CloseHandle(event);
...
#  elif TARGET_OS_MAC
  ...
  // Set callback
  auto callback = [](CFHostRef theHost,
...
#  else
  ...
  std::thread resolve_thread(
      [state]()
      {
        auto thread_result = getaddrinfo(state->node.c_str(),
                                         state->service.c_str(),
                                         &state->hints,
                                         &state->info);
  ...
  auto finished = state->result_cv.wait_for(lock,
...
#  endif
  ...
  if (getaddrinfo_with_timeout(
          node, service.c_str(), &hints, &result, timeout_sec))
  {
...
inline bool zstd_compressor::compress(const char* data,
                                      size_t data_length,
                                      bool last,
                                      Callback callback)
{
...
inline bool zstd_decompressor::decompress(const char* data,
                                          size_t data_length,
                                          Callback callback)
{
  std::array<char, CPPHTTPLIB_COMPRESSION_BUFSIZ> buff {};
...
template<typename T>
inline ReadContentResult read_content_chunked(Stream& strm,
                                              T& x,
...
inline ssize_t write_request_line(Stream& strm,
                                  const std::string& method,
                                  const std::string& path)
{
  std::string s = method;
...
inline bool write_content(Stream& strm,
                          ...
                          Error& error)
{
  return write_content_with_progress<T>(
      strm, content_provider, offset, length, is_shutting_down, nullptr, error);
}
...
inline void parse_query_text(const char* data, std::size_t size, Params& params)
{
  ...
  split(data,
        ...
        [&](const char* b, const char* e)
        {
          ...
          if (!key.empty()) {
            params.emplace(decode_query_component(key),
                           decode_query_component(val));
          }
        });
}
        ...
        [&](const char* b, const char* e)
        {
          ...
          if (q_pos != std::string::npos) {
...
#ifdef CPPHTTPLIB_NO_EXCEPTIONS
            {
              ...
              // Check if conversion was successful and entire string was
              // consumed
              if (iss.fail() || !iss.eof()) {
                has_invalid_entry = true;
                return;
    ...
    bool is_proxy = false)
{
  ...
  auto field = "Digest username=\"" + username + "\", realm=\""
      + auth.at("realm") + "\", nonce=\"" + auth.at("nonce") + "\", uri=\""
                      ...
                      std::vector<std::string>& addrs)
{
  ...
  if (detail::getaddrinfo_with_timeout(
          hostname.c_str(), nullptr, &hints, &result, 0))
  {
#if defined __linux__ && !defined __ANDROID__
    res_init();
#endif
...
inline bool SocketStream::wait_readable() const
{
  ...
  time_t read_timeout_sec;
  time_t read_timeout_usec;
  calc_actual_timeout(max_timeout_msec_,
                      duration(),
                      read_timeout_sec_,
                      read_timeout_usec_,
                      read_timeout_sec,
...
inline std::string make_host_and_port_string(const std::string& host,
                                             int port,
                                             bool is_ssl)
{
...
template<typename T>
inline bool check_and_write_headers(Stream& strm,
                                    Headers& headers,
                                    T header_writer,
                                    Error& error)
{
...
inline bool Server::parse_request_line(const char* s, Request& req) const
{
  ...
  thread_local const std::set<std::string> methods {"GET",
                                                    "HEAD",
                                                    "POST",
                                                    "PUT",
                                                    "DELETE",
  ...
  if (req.is_multipart_form_data()) {
    ...
    out = [&](const char* buf, size_t n, size_t /*off*/, size_t /*len*/)
    {
      return multipart_form_data_parser.parse(
          buf, n, multipart_header, multipart_receiver);
    };
  } else {
...
inline bool Server::check_if_not_modified(const Request& req,
                                          Response& res,
                                          const std::string& etag,
                                          time_t mtime) const
{
...
inline bool Server::check_if_range(Request& req,
                                   const std::string& etag,
                                   time_t mtime) const
{
...
inline bool Server::listen_internal()
{
...

  {
    ...
    while (svr_sock_ != INVALID_SOCKET) {
      ...
      detail::set_socket_opt_time(
          sock, SOL_SOCKET, SO_RCVTIMEO, read_timeout_sec_, read_timeout_usec_);
      detail::set_socket_opt_time(sock,
                                  SOL_SOCKET,
                                  SO_SNDTIMEO,
                                  write_timeout_sec_,
                                  write_timeout_usec_);
  ...
  if (detail::expect_content(req)) {
    // Content reader handler
    {
      ContentReader reader(
          ...
          {
            auto result =
                read_content_with_content_receiver(strm,
                                                   req,
                                                   res,
                                                   nullptr,
                                                   std::move(header),
    ...
    const std::vector<std::string>& trusted_proxies)
{
  ...
  for (size_t i = 0; i < ip_list.size(); ++i) {
    ...
    auto is_trusted_proxy =
        std::any_of(trusted_proxies.begin(),
                    trusted_proxies.end(),
      ...
      if (is_ssl()) {
        ...
        if (!proxy_host_.empty() && proxy_port_ != -1) {
          ...
          if (!scli.connect_with_proxy(
                  socket_, req.start_time_, res, success, error))
          {
            if (!success) {
              output_error_log(error, &req);
            }
...
inline void ClientImpl::prepare_default_headers(Request& r,
                                                bool for_stream,
                                                const std::string& ct)
{
  (void)for_stream;
    ...
    const std::string& content_type)
{
  ...
  if (is_ssl() && handle.connection_->ssl) {
    handle.socket_stream_ =
        detail::make_unique<detail::SSLSocketStream>(handle.connection_->sock,
                                                     handle.connection_->ssl,
                                                     read_timeout_sec_,
                                                     read_timeout_usec_,
                                                     write_timeout_sec_,
                                                     write_timeout_usec_);
    ...
    handle.socket_stream_ =
        detail::make_unique<detail::SocketStream>(handle.connection_->sock,
                                                  read_timeout_sec_,
                                                  read_timeout_usec_,
                                                  write_timeout_sec_,
                                                  write_timeout_usec_);
  ...
  if (!detail::check_and_write_headers(
          strm, req.headers, header_writer_, handle.error))
  {
    handle.response.reset();
    return handle;
  }
                                                                 ...
                                                                 size_t len)
{
  ...
  while (true) {
    auto n = detail::read_body_content(
        stream_, body_reader_, compressed_buf, sizeof(compressed_buf));
    ...
    bool decompress_ok =
        decompressor_->decompress(compressed_buf,
...
inline void ClientImpl::StreamHandle::parse_trailers_if_needed()
{
  ...
  if (!detail::parse_trailers(
          line_reader, response->trailers, response->headers))
  {
    return;
  }
}
...
inline ssize_t ChunkedDecoder::read_payload(char* buf,
                                            size_t len,
                                            size_t& out_chunk_offset,
                                            size_t& out_chunk_total)
{
                                       ...
                                       Error& error)
{
  ...
  if (!is_ssl() && !proxy_host_.empty() && proxy_port_ != -1) {
    auto req2 = req;
    req2.path = "http://"
        + detail::make_host_and_port_string(host_, port_, false) + req.path;
...
inline bool ClientImpl::redirect(Request& req, Response& res, Error& error)
{
  ...
  return create_redirect_client(
      next_scheme, next_host, next_port, req, res, path, location, error);
}
                                                    ...
                                                    Error& error) const
{
  ...
  if (req.is_chunked_content_provider_) {
    ...
    return detail::write_content_with_progress(strm,
                                      ...
                                      Error& error)
{
  ...
  // Request line and headers
  {
    ...
    if (!detail::check_and_write_headers(
            bstrm, req.headers, header_writer_, error))
    {
      output_error_log(error, &req);
      return false;
    }
...
{
  return Post(
      path, Headers(), std::move(content_provider), content_type, progress);
}
...
inline STACK_OF(X509_NAME)
    * SSLServer::extract_ca_names_from_x509_store(X509_STORE* store)
{
  if (!store) {
    return nullptr;
  }
    ...
    : ClientImpl(host, port, client_cert_path, client_key_path)
{
  ...
  if (!client_cert_path.empty() && !client_key_path.empty()) {
    ...
    if (SSL_CTX_use_certificate_file(
            ctx_, client_cert_path.c_str(), SSL_FILETYPE_PEM)
            != 1
        || SSL_CTX_use_PrivateKey_file(
               ctx_, client_key_path.c_str(), SSL_FILETYPE_PEM)
            != 1)
    {
...
{
  return cli_->Post(
      path, headers, body, content_type, std::move(content_receiver), progress);
}
...
{
  return cli_->Put(
      path, headers, body, content_type, content_receiver, progress);
}
