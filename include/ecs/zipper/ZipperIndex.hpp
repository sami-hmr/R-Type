 ...
 * @brief Iterator that zips multiple containers and includes the current index in the result.
 * @tparam Containers Variadic template parameter pack of container types to iterate over.
 *
 * This iterator extends ZipperIterator by prepending the current iteration index to the tuple
 * of values. It allows tracking both the position in the iteration and the actual values
 * from each container.
 *
 * @note The index represents the logical position in the iteration, which may differ from
 *       the actual container indices due to skipped empty optional values.
 ...
 *     std::cout << "Position " << index << ": " << val1 << ", " << val2 << std::endl;
 * }
 * @endcode
 */
...
public:
  ...
  /**
   * @brief Constructs a zipper index iterator by forwarding arguments to the base iterator.
   ...
   * automatically skips positions where any container has empty optional values.
   */
  ...
  /**
   * @brief Dereference operator. Returns tuple with index and values from all containers.
   * @return Tuple where the first element is the current index, followed by values from each container.
   */
  ValueType operator*()
  {
  ...
  /**
   * @brief Arrow operator. Returns tuple with index and values from all containers.
...
private:
  Base _base; ///< Underlying ZipperIterator that provides the core iteration logic.
};

/**
 * @class ZipperIndex
 * @brief Range adapter that zips multiple containers and includes indices in the iteration.
 * @tparam Containers Variadic template parameter pack of container types to zip together.
 *
 * This class extends the functionality of Zipper by including the current iteration index
 * in each yielded tuple. It's useful when you need to know the position during iteration,
 * for example when processing entities by their index or performing indexed operations.
 *
 * @note The index is the logical iteration position, not necessarily the container index,
 *       since positions with empty optionals are skipped.
 ...
 *     std::cout << "Entity " << idx << " at (" << pos.x << "," << pos.y << ")" << std::endl;
 *     pos.x += vel.dx;
 *     pos.y += vel.dy;
 * }
 * @endcode
...
public:
   ...
   * The zipper will iterate up to the size of the smallest container, yielding tuples
   * that include the iteration index.
   */
   ...
   *
   * The returned iterator will yield tuples of (index, value1, value2, ...) where
   * index starts at 0 and increments for each valid position.
   */
  Iterator begin()
  {
