...
class ZipperIterator {
    ...
    using iterator = std::conditional_t<std::is_const_v<Container>, typename Container::cit, typename Container::it>;

    template <class Container>
    using it_ref = typename iterator<Container>::reference;

    template<class Container>
    using value = std::conditional_t<std::is_const_v<Container>, typename Container::true_cref, typename Container::true_ref>;
    ...
    ZipperIterator(iterator_tuple const &it_tuple, std::size_t max, std::size_t index = 0):
        current_(it_tuple), max_(max), idx_(index) {
        while (this->idx_ < this->max_ && !this->all_set(_seq)) {
            this->incr_all(_seq);
        }
    }

    ZipperIterator(ZipperIterator const &z): current_(z.current_), max_(z.max_), idx_(z.idx_) {}
    ...
    bool operator ==(ZipperIterator const &rhs) {
...
class Zipper {
    ...
    static iterator_tuple compute_end_(Containers &...containers) {
        return std::make_tuple(containers.begin() + compute_size_(containers...)...);
    }
