...
struct InteractionBorders
{
  ...
  InteractionBorders(Registry& r, JsonObject const& e)
      ...
      , in_zone(get_value_copy<std::unordered_set<Registry::Entity>>(r, e, "in_zone").value())
  {
  }
  ...
  DEFAULT_BYTE_CONSTRUCTOR(
      ...
      ([](bool e, double r, std::vector<Registry::Entity> const& i)
       { return InteractionBorders(e, r, std::unordered_set(i.begin(), i.end())); }),
      parseByte<bool>(),
      parseByte<double>(),
      parseByteArray<Registry::Entity>(parseByte<Registry::Entity>()))

  DEFAULT_SERIALIZE(
      type_to_byte(enabled),
      type_to_byte(radius),
      [this]() {std::vector<Registry::Entity> v(in_zone.begin(), in_zone.end());
            return vector_to_byte(v,
