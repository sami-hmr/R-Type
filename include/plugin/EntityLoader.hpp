...
class EntityLoader {
    public:
    EntityLoader(Registery &registery): registery_(registery) {}
    ...
    void load_file(std::string const &filepath) {
        ...
        if (infi.fail()) {
            std::cerr << "failed to open file \"" << filepath << "\": " << std::strerror(errno) << std::endl;
        }
        ...
        if (result.index() == ERROR) {
            ...
            try {
                ...
                std::cerr << "Parsing \"" << filepath << "\": missing \"entities\" field" << std::endl;
            } catch (std::bad_variant_access &) {
                std::cerr << "Parsing \"" << filepath << "\": invalid \"entities\" value" << std::endl;
            }
        }
        ...
        for (auto const &[key, sub_config] : config) {
            ...
            try {
                if (!this->loaders_.contains(plugin)) {
                    #if __linux__
                    this->loaders_.emplace(plugin, std::make_unique<DlLoader<IPlugin>>("plugins/" + plugin));
                    #endif
                }
            ...
            try {
                if (!this->plugins_.contains(plugin)) {
                    this->plugins_.emplace(plugin,
                        this->loaders_.at(plugin)->getInstance("entry_point", this->registery_, *this));
                }
            } catch (LoaderException const &e) {
                std::cerr << e.what() << std::endl;
            }
            this->plugins_.at(plugin)->setComponent(new_entity, comp, sub_config.value);
        }
    }
